...
getEmojiHTML: function(code, symbol, enabled) {
  ...
  if (Emoji.cssEmoji[code] != undefined) {
    ...
    return '<img'+editable+' src="http://vk.com/images/blank.gif" emoji="'+code+'" '+(symbol ? 'alt="'+symbol+'"' : symbol)+' class="emoji_css" style="background-position: 0px '+num+'px;" />';
  } else {
    if (!Emoji.imgEmoji[code] && symbol && !enabled) {
      return symbol;
    } else {
      return '<img class="emoji" '+(symbol ? 'alt="'+symbol+'"' : '')+' src="http://vk.com/images/emoji'+(window.devicePixelRatio >= 2 ? '_2x' : '')+'/'+code+'.png" />';
    }
  }
},
...
ttClick: function(optId, obj, needHide, needShow, ev) {
  ...
  if (!tt) {
    ...
    if (opts.rPointer) {
      shopEvents += ' onmouseover="addClass(this.parentNode.parentNode.parentNode, \'emoji_shop_over\');"  onmouseout="removeClass(this.parentNode.parentNode.parentNode, \'emoji_shop_over\');"';
    }
    tabs += '</span></span>';
    if (!opts.noStickers) {
      tabs += '<a class="fl_r emoji_shop" '+shopEvents+'><div class="emoji_sprite emoji_shop_icon"></div></a>';
    }
  ...
  var topShift = opts.topShift || 199;
  if (Emoji.shown) {
...
updateBuyBtn: function(obj, btnText, updateObj) {
  if (updateObj) {
    obj.parentNode.replaceChild(cf(btnText), obj);
  } else {
...
buyStickers: function(packId, ev, obj, hash) {
  ...
  ajax.post('/al_im.php', {act: 'a_stickers_buy', pack_id: packId, hash: hash, peer: peer}, {
    onDone: function(text, newStickers, btnText, updateObj) {
      if (hasClass(obj, 'im_sticker_act_blue')) {
        Emoji.updateBuyBtn(obj, btnText, updateObj);
      }
      if (cur.stickersDepEl) {
        Emoji.updateBuyBtn(cur.stickersDepEl, btnText, updateObj);
      }
